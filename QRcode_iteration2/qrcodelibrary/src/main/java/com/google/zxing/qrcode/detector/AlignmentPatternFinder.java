            ...
            if(stateCount[0] > maxCount) {
                ...
                if(i != maxI && stateCount[1] <= maxCount) {
                    ...
                    if(stateCount[2] > maxCount) {
                        ...
                        return 5 * Math.abs(stateCountTotal - originalStateCountTotal) >= 2 * originalStateCountTotal?0.0F / 0.0F:(this.foundPatternCross(stateCount)?centerFromEnd(stateCount, i):0.0F / 0.0F);
                    }
                } else {
                    return 0.0F / 0.0F;
                }
    ...
    private AlignmentPattern handlePossibleCenter(int[] stateCount, int i, int j) {
        ...
        if(!Float.isNaN(centerI)) {
            ...
            for(int point = 0; point < max; ++point) {
                AlignmentPattern center = (AlignmentPattern)this.possibleCenters.elementAt(point);
                if(center.aboutEquals(estimatedModuleSize, centerI, centerJ)) {
                    return new AlignmentPattern(centerJ, centerI, estimatedModuleSize);
                }
            }

            AlignmentPattern var11 = new AlignmentPattern(centerJ, centerI, estimatedModuleSize);
            this.possibleCenters.addElement(var11);
