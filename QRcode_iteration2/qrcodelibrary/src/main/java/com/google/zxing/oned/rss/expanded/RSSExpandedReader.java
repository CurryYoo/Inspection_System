...
public final class RSSExpandedReader extends AbstractRSSReader {
    ...
    private boolean checkChecksum() {
        ...
        for(checkCharacterValue = 1; checkCharacterValue < this.pairs.size(); ++checkCharacterValue) {
            ExpandedPair currentPair = (ExpandedPair)this.pairs.elementAt(checkCharacterValue);
    ...
    private static int getNextSecondBar(BitArray row, int initialPos) {
        ...
        boolean current;
        for(current = row.get(initialPos); currentPos < row.size && row.get(currentPos) == current; ++currentPos) {
            ;
        }

        for(current = !current; currentPos < row.size && row.get(currentPos) == current; ++currentPos) {
            ;
        }
    ...
    private boolean checkPairSequence(Vector previousPairs, FinderPattern pattern) throws NotFoundException {
        ...
        if(currentSequenceLength > this.currentSequence.length) {
            ...
            int i;
            for(i = 0; i < previousPairs.size(); ++i) {
                this.currentSequence[i] = ((ExpandedPair)previousPairs.elementAt(i)).getFinderPattern().getValue();
            }

            this.currentSequence[currentSequenceLength - 1] = pattern.getValue();

            for(i = 0; i < FINDER_PATTERN_SEQUENCES.length; ++i) {
    ...
    private void findNextPair(BitArray row, Vector previousPairs, int forcedOffset) throws NotFoundException {
        ...
        if(forcedOffset >= 0) {
            ...
            ExpandedPair searchingEvenPair = (ExpandedPair)previousPairs.lastElement();
            rowOffset = searchingEvenPair.getFinderPattern().getStartEnd()[1];
        }

        boolean var13 = previousPairs.size() % 2 != 0;
    ...
    private FinderPattern parseFoundFinderPattern(BitArray row, int rowNumber, boolean oddPattern) {
        ...
        int counters;
        if(oddPattern) {
            for(counters = this.startEnd[0] - 1; counters >= 0 && !row.get(counters); --counters) {
                ;
            }

            ++counters;
            firstCounter = this.startEnd[0] - counters;
            start = counters;
            end = this.startEnd[1];
        } else {
            start = this.startEnd[0];

            for(counters = this.startEnd[1] + 1; row.get(counters) && counters < row.size; ++counters) {
                ;
            }

            end = counters;
            firstCounter = counters - this.startEnd[1];
        }

        int[] var11 = this.decodeFinderCounters;

        int value;
        for(value = var11.length - 1; value > 0; --value) {
            var11[value] = var11[value - 1];
        }

        var11[0] = firstCounter;

        try {
            value = parseFinderValue(var11, FINDER_PATTERNS);
    ...
    DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean isOddPattern, boolean leftChar) throws NotFoundException {
        ...
        if(leftChar) {
            ...
            for(int elementWidth = counters.length - 1; numModules < elementWidth; --elementWidth) {
                int oddCounts = counters[numModules];
                counters[numModules] = counters[elementWidth];
                counters[elementWidth] = oddCounts;
                ++numModules;
            }
        }

        byte var26 = 17;
        float var27 = (float)count(counters) / (float)var26;
        int[] var28 = this.oddCounts;
        ...
        for(weightRowNumber = 0; weightRowNumber < counters.length; ++weightRowNumber) {
            float oddSum = 1.0F * (float)counters[weightRowNumber] / var27;
            oddChecksumPortion = (int)(oddSum + 0.5F);
            ...
            if((weightRowNumber & 1) == 0) {
                var28[evenChecksumPortion] = oddChecksumPortion;
                oddRoundingErrors[evenChecksumPortion] = oddSum - (float)oddChecksumPortion;
            } else {
                evenCounts[evenChecksumPortion] = oddChecksumPortion;
                evenRoundingErrors[evenChecksumPortion] = oddSum - (float)oddChecksumPortion;
            }
        }

        this.adjustOddEvenCounts(var26);
        weightRowNumber = 4 * pattern.getValue() + (isOddPattern?0:2) + (leftChar?0:1) - 1;
        int var29 = 0;
        oddChecksumPortion = 0;

        int evenSum;
        for(evenChecksumPortion = var28.length - 1; evenChecksumPortion >= 0; --evenChecksumPortion) {
            if(isNotA1left(pattern, isOddPattern, leftChar)) {
                evenSum = WEIGHTS[weightRowNumber][2 * evenChecksumPortion];
                oddChecksumPortion += var28[evenChecksumPortion] * evenSum;
            }

            var29 += var28[evenChecksumPortion];
        }
        ...
        if((var29 & 1) == 0 && var29 <= 13 && var29 >= 4) {
            group = (13 - var29) / 2;
            int oddWidest = SYMBOL_WIDEST[group];
            int evenWidest = 9 - oddWidest;
            int vOdd = RSSUtils.getRSSvalue(var28, oddWidest, true);
