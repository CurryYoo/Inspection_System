...
public final class RSS14Reader extends AbstractRSSReader {
    ...
    private static Result constructResult(Pair leftPair, Pair rightPair) {
        ...
        for(int leftPoints = 0; leftPoints < 13; ++leftPoints) {
            int rightPoints = buffer.charAt(leftPoints) - 48;
            checkDigit += (leftPoints & 1) == 0?3 * rightPoints:rightPoints;
        }
        ...
        ResultPoint[] var9 = leftPair.getFinderPattern().getResultPoints();
        ResultPoint[] var10 = rightPair.getFinderPattern().getResultPoints();
        return new Result(String.valueOf(buffer.toString()), (byte[])null, new ResultPoint[]{var9[0], var9[1], var10[0], var10[1]}, BarcodeFormat.RSS14);
    }
    ...
    private Pair decodePair(BitArray row, boolean right, int rowNumber, Hashtable hints) {
        try {
            int[] re = this.findFinderPattern(row, 0, right);
            FinderPattern pattern = this.parseFoundFinderPattern(row, rowNumber, right, re);
            ...
            if(resultPointCallback != null) {
                float outside = (float)(re[0] + re[1]) / 2.0F;
                if(right) {
                    outside = (float)(row.getSize() - 1) - outside;
                }

                resultPointCallback.foundPossibleResultPoint(new ResultPoint(outside, (float)rowNumber));
            }

            DataCharacter outside1 = this.decodeDataCharacter(row, pattern, true);
            DataCharacter inside = this.decodeDataCharacter(row, pattern, false);
            return new Pair(1597 * outside1.getValue() + inside.getValue(), outside1.getChecksumPortion() + 4 * inside.getChecksumPortion(), pattern);
    ...
    private DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean outsideChar) throws NotFoundException {
        ...
        float var23 = (float)count(counters) / (float)numModules;
        int[] var24 = this.oddCounts;
        ...
        for(oddSum = 0; oddSum < counters.length; ++oddSum) {
            float oddChecksumPortion = (float)counters[oddSum] / var23;
            evenChecksumPortion = (int)(oddChecksumPortion + 0.5F);
            ...
            evenSum = oddSum >> 1;
            if((oddSum & 1) == 0) {
                var24[evenSum] = evenChecksumPortion;
                oddRoundingErrors[evenSum] = oddChecksumPortion - (float)evenChecksumPortion;
            } else {
                evenCounts[evenSum] = evenChecksumPortion;
                evenRoundingErrors[evenSum] = oddChecksumPortion - (float)evenChecksumPortion;
            }
        }
        ...
        for(evenChecksumPortion = var24.length - 1; evenChecksumPortion >= 0; --evenChecksumPortion) {
            var25 *= 9;
            var25 += var24[evenChecksumPortion];
            oddSum += var24[evenChecksumPortion];
        }
        ...
        checksumPortion = var25 + 3 * evenChecksumPortion;
        int group;
        int oddWidest;
        int evenWidest;
        int vOdd;
        ...
        if(outsideChar) {
            if((oddSum & 1) == 0 && oddSum <= 12 && oddSum >= 4) {
                ...
                vOdd = RSSUtils.getRSSvalue(var24, oddWidest, false);
            ...
            evenWidest = 9 - oddWidest;
            vOdd = RSSUtils.getRSSvalue(var24, oddWidest, true);
    ...
    private FinderPattern parseFoundFinderPattern(BitArray row, int rowNumber, boolean right, int[] startEnd) throws NotFoundException {
        ...
        int value;
        for(value = counters.length - 1; value > 0; --value) {
