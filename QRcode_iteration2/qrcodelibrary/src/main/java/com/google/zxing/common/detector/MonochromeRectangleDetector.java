...
public final class MonochromeRectangleDetector {
    ...
    public ResultPoint[] detect() throws NotFoundException {
        ...
        int top1 = (int)pointA.getY() - 1;
        ResultPoint pointB = this.findCornerFromCenter(halfWidth, -deltaX, left, width, halfHeight, 0, top1, height, halfHeight >> 1);
        int left1 = (int)pointB.getX() - 1;
        ResultPoint pointC = this.findCornerFromCenter(halfWidth, deltaX, left1, width, halfHeight, 0, top1, height, halfHeight >> 1);
        int right = (int)pointC.getX() + 1;
        ResultPoint pointD = this.findCornerFromCenter(halfWidth, 0, left1, right, halfHeight, deltaY, top1, height, halfWidth >> 1);
        int bottom = (int)pointD.getY() + 1;
        pointA = this.findCornerFromCenter(halfWidth, 0, left1, right, halfHeight, -deltaY, top1, bottom, halfWidth >> 2);
